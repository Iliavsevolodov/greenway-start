const screens=[...document.querySelectorAll('.screen')],tasks=[...document.querySelectorAll('[data-task]')],nextBtns=[...document.querySelectorAll('[data-next]')],backBtns=[...document.querySelectorAll('[data-back]')],stepLabel=document.getElementById('stepLabel'),stepTitle=document.getElementById('stepTitle'),dots=document.getElementById('dots'),resetBtn=document.getElementById('resetBtn'),toast=document.getElementById('toast'),confetti=document.getElementById('confetti'),openCards=[...document.querySelectorAll('.open')];
const KEY='greenway-start-yellow-v1',TITLES=['Командный чат','Компания Greenway','Приложение Greenway','Следующий шаг'];let current=0;
...
function note(t){toast.textContent=t;toast.classList.add('show');clearTimeout(note.t);note.t=setTimeout(()=>toast.classList.remove('show'),1500)}
function party(){confetti.innerHTML='';for(let i=0;i<28;i++){const p=document.createElement('span');p.className='confetti-piece';p.textContent=['✨','●','◆','✳'][Math.floor(Math.random()*4)];p.style.left=Math.random()*100+'%';p.style.color=['#f6c400','#15120d','#2f7d4a','#ffe67a'][Math.floor(Math.random()*4)];p.style.animationDelay=Math.random()*.25+'s';confetti.appendChild(p)}setTimeout(()=>confetti.innerHTML='',1700)}
